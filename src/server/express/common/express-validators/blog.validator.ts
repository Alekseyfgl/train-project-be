...
const websiteUrlValidator = body('websiteUrl').isString().trim().isLength({ min: 1, max: 100 }).matches('^https://([a-zA-Z0-9_-]+\\.)+[a-zA-Z0-9_-]+(\\/[a-zA-Z0-9_-]+)*\\/?$\n').withMessage('Incorrect description!');

export const blogPostValidation = () => [nameValidator, descriptionValidator, websiteUrlValidator, inputModelValidator];
